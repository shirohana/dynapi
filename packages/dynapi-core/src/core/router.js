...
import { RouterOptions, Utils } from '../common'
...
export default class Router extends Anchor {
  ...
  set (filename, exports) {
    ...
    switch (type) {
      ...
      case types.catcher:
        this.context.ensure(paths).catcher.set(name, { name, filename })
        break
  ...
  unset (filename) {
    const { type, name, paths } = this.parse(filename)
    const types = this.options.prefixes
    const route = this.context.find(paths)

    if (route !== null) {
      switch (type) {
        case types.middleware:
          route.middleware.unset(name)
          break

        case types.parameter:
          route.parameter.unset(name)
          break

        case types.responser:
          route.responser.unset(name.toUpperCase())
          break
      }
    }
  ...
  parseFilename (filename) {
    ...
    let type, name, paths
    const isResponser = !Object.values(types).includes(prefix)
