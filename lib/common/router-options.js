...
RouterOptions.from = function (opt) {
  ...
  if (RouterOptions.defaults.dev) {
    assert.equal('string', typeof opt.srcdir)
    assert.equal('string', typeof opt.routesdir)
  }

  // Set up dirs
  options.rootdir = hasValue(opt.rootdir) ? resolve(opt.rootdir) : RouterOptions.defaults.rootdir
  options.srcdir = join(options.rootdir, opt.srcdir)
  options.routesdir = join(options.srcdir, opt.routesdir)

  // Final check
  if (RouterOptions.defaults.dev) {
    assert.equal('object', typeof options.prefixes)
  ...
  if (Array.isArray(opt.aliases)) {
    opt.aliases.forEach(alias => {
      ...
      aliases[from] = join(options.srcdir, to)
    })
  }
