...
export default class Builder extends Tapable {
  ...
  async build () {
    ...
    await remove(this.options.buildDir)
    await mkdirp(this.options.buildApiDir)

    const apiFiles = await glob('**/*.js', { cwd: this.options.apiDir, absolute: true })
    const transformPromises = apiFiles.map(async (filename) => {
      try {
        const transformed = this.transform(filename)
        const buildFilename = this.dynapi.redirectTo('buildApi', filename)
  ...
  transform (filename) {
    ...
    const requirer = (name) => this.require(name, filename)
    ...
    if (this.options.dev) {
      ...
      imports.forEach(({ source }) => {
        if (source.startsWith('~controller')) {
          const key = this.dynapi.resolve(source)
  ...
  require (name, requior) {
    if (name.startsWith('~')) {
      return this.fetch(this.dynapi.resolve(name))
    } else if (name.startsWith('.')) {
      return this.fetch(this.dynapi.resolve(name, { from: dirname(requior) }))
    } else {
      return this.dynapi.nativeRequire(name)
    }
  }
}
