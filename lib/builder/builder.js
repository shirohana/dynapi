...
export default class Builder extends Tapable {
  ...
  fetch (filepath) {
    const buildFilepath = this.dynapi.redirectTo('build', filepath)

    if (this.handlers[buildFilepath]) {
      return this.handlers[buildFilepath]
    } else {
      let exports
      ...
      try {
        ...
        Messages.printError(err, filepath)
        exports = Builder.getBuildFailureHandler(err)
      }

      return (this.handlers[buildFilepath] = exports)
    }
  }
