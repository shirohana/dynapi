  ...
  watch () {
    ...
    listeners.forEach(({ event, verb, callback }) => {
      this.watcher.on(event, filepath => {
        debug(chalk`{magenta ${relative(this.options.srcDir, filepath)}} has been {red ${verb}}.`)
    ...
    debug('Start watching %O', patterns.map(p => relative(this.options.srcDir, p)))
  ...
  fetch (filepath) {
    ...
    if (filepath.startsWith(this.options.routesDir)) {
      this.dynapi.renderer.add({
        filepath: relative(this.options.routesDir, filepath),
        exports
      })
    } else {
      this.add(filepath)
