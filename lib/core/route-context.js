...
class Route {
  ...
  clean () {
    ...
    this.children.forEach(route => route.clean())
  }

  prepare () {
    this.isPreparing = true
    this.children.forEach(route => route.prepare())
  }

  bake () {
    this.children.forEach(route => route.bake())
  ...
  get children () {
    return [...Object.values(this.subRoutes), ...Object.values(this.subParamRoutes)]
  }
  ...
  get parameter () {
    return (this._p = this._p || {
      ...
      _touch: (key) => {
        ...
        // Update all child route which contains `key` param
        this.children.forEach(route => route.parameter._touch(key))
      }
    })
  }
