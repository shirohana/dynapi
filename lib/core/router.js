...
export default class Router {
  constructor (renderer) {
    ...
    this._symbolValues = Object.values(this.symbol)
  ...
  calculate (method, url) {
    ...
    while ((key = keys.shift()) !== undefined) {
      ...
      if (typeof route.subRoute[key] === 'object') {
        ...
        for (let paramRoute of Object.values(route.subParamRoute)) {
      ...
      // Process parameters
      if (args) {
        queue.push(...Object.entries(params).map(([name, route], index) => ({
  ...
  parseFilename (filename) {
    ...
    if (!paths) {
      ...
      return {
        ...
        paths: paths.map(p => p[0] === '(' ? p.slice(1, -1) : kebabCase(p))
      }
    }
  }
}
...
function filenamePatternOf ({ symbol, extensions }) {
  const prefix = Object.values(symbol).concat('[a-z]+').join('|')
