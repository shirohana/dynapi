...
export default class Router {
  ...
  generate (files) {
    const root = Routes.createRoute({ path: '/' })

    files.forEach((file) => {
      const keys = Routes.parseKeys(file)
      const paths = keys.slice(0, -1)
      const type = keys.slice(-1)[0].toLowerCase()

      const filename = join(this.options.build.routesDir, file)

      if (type === 'middleware') {
        const middleware = Routes.createMiddleware({ filename })
        Routes.ensureRoutes(root, paths).middleware = middleware
      } else if (type === 'param') {
        ...
        if (paramKey[0] !== ':') {
          ...
          Object.assign(Routes.ensureRoutes(root, paths), {
            param: Routes.createParam({ filename, name: paramKey.substring(1) })
          })
        }
      } else {
        ...
        if (priority < 0) {
          ...
          let method = Routes.createMethod({ filename })
          Routes.ensureRoutes(root, paths).methods[type.toUpperCase()] = method
        }
      }
