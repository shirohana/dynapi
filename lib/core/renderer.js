...
export default class Renderer extends Tapable {
  ...
  async createRenderer (handlers = {}) {
    this._status = Status.RENDERER_CREATING | (this._status & ~Status.RENDERER_CREATED)
  ...
  async renderRoute (url, context) {
    ...
    // Prevent from calling `Times.currentTime` to frequently
    if (debug.enabled) {
    ...
    try {
      ...
      if (typeof err.status === 'number' && !res.headersSent) {
