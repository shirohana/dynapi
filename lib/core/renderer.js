...
import { RendererOptions } from '../common'
...
export default class Renderer extends Anchor {
  ...
  handleError (err, req, res, layers) {
    ...
    while (!catcher && layers.length > 0) {
      ...
      let { catches } = handle
      if (catches) {
        if (typeof catches === 'function' && err instanceof catches) {
          break
        } else if (Array.isArray(catches) && catches.findIndex(proto => err instanceof proto) > -1) {
          break
        } else {
          catcher = null
          continue
