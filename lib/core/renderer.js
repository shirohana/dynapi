...
export default class Renderer extends Tapable {
  ...
  async createRenderer (handlers = {}) {
    if (this.options.dev) {
      ...
      const files = await glob('**/*.js', { cwd: this.options.apiDir })
  ...
  async renderRoute (url, context) {
    ...
    const r = (filename) => this.dynapi.relativeFrom('api', this.dynapi.redirectTo('api', filename))
    ...
    const handlerChain = routes.reduce(async (resolving, route) => {
      ...
      if (timeout > 0) {
        next = await Promise.race([
          ...
          new Promise((resolve, reject) => setTimeout(() => {
            ...
            reject(
              `Error: Timeout exceeds \`${timeout}\` without calling next()\n` +
              `    at (${this.dynapi.redirectTo('api', filename)}:1:1)`
            )
          }, timeout))
        ])
      }
      ...
      if (next instanceof Error) {
        next.filename = this.dynapi.redirectTo('api', filename)
      } else if (typeof next === 'string') {
        next = `Error: ${next}\n` +
          `    at exports.default (${this.dynapi.redirectTo('api', filename)}:1:1)`
      }
